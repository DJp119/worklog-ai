...
- New `slack_user_links(id, org_id→organizations ON DELETE CASCADE, user_id→users ON DELETE CASCADE, slack_user_id, slack_team_id, unique(slack_team_id,slack_user_id), unique(user_id))` — a user should have at most one Slack link.
...
- `team_closure_after_reparent()` (AFTER UPDATE OF parent_team_id): only runs if `NEW.parent_team_id IS DISTINCT FROM OLD.parent_team_id`. Cuts old-ancestor↔subtree edges, and stitches new-ancestor × subtree with `super.depth+sub.depth+1`.
...
**`server/src/routes/integrations/jira.ts`** — `/api/integrations/jira/connect` (302 to Atlassian 3LO: scopes `read:jira-work read:jira-user offline_access`, signed `state`), `/callback` (exchange code, call `accessible-resources` for available sites. If multiple sites are returned, redirect the user to a selection page in the client to choose which `cloudId` and `siteUrl` to link, then encrypt and store access/refresh tokens in `user_integrations` with `config={cloudId,siteUrl}`), `/disconnect`. Handle **rotating refresh tokens** (store the new refresh token on every refresh; proactive refresh when `<60s` left) via a shared `getJiraClient(userId)` in `server/src/lib/jiraAdapter.ts`.
...
**`server/src/lib/githubApp.ts`** — GitHub App install callback: since GitHub App installation flows do not support a `state` parameter, use a secure session cookie to store the initiating user's current `org_id` before redirecting them to GitHub. Upon callback, read this `org_id` from the session to store `installation.id` in `org_integrations(provider='github_app')` and associate it with the correct organization. Mint installation tokens for org-level reads.
