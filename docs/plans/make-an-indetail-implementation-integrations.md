...
- `org_members(id, org_id, user_id, role org_role, unique(org_id,user_id))` — `org_role` enum = `member|admin|owner`
- `departments(id, org_id, name)`
- `teams(id, org_id, department_id→departments ON DELETE SET NULL, parent_team_id→teams ON DELETE SET NULL, name, timestamps)`
- `team_members(id, team_id, user_id, role team_role, unique(team_id,user_id))` — `team_role` enum = `member|manager|admin|owner`
- `team_closure(ancestor_id, descendant_id, depth, pk(ancestor_id,descendant_id))` + indexes on both columns
...
- `goals(id, org_id, scope, team_id, department_id, parent_goal_id→goals ON DELETE SET NULL, title, description, status, period, start_date, due_date, progress numeric(5,2) default 0, progress_mode text default 'manual', rollup_weight numeric default 1, created_by→users, timestamps)` + `CHECK` that scope matches the target column.
- `goal_assignees(goal_id, user_id, assigned_by, assigned_at, pk(goal_id,user_id))`
- `goal_key_results(id, goal_id, title, metric_type, start_value numeric default 0, target_value, current_value default 0, unit, weight numeric default 1, sort_order, timestamps)` — **`start_value` and `weight` are model fixes** required by the progress formula.
- `goal_links(id, goal_id, provider jira|github, link_type, external_id, external_key, external_url, title, state, is_done bool, weight numeric default 1, created_by→users, org_id→organizations, metadata jsonb, timestamps, unique(goal_id,provider,external_id))` + index `(provider, external_id)`. **`created_by` + `org_id` are model fixes** that make webhook routing O(1) and safe.
- `goal_updates(id, goal_id, user_id, progress, status, note, created_at)` — check-in history.
...
- New `org_integrations(id, org_id, provider slack|github_app, external_install_id, bot_token_enc, webhook_secret, config jsonb, installed_by→users, is_active bool, timestamps, unique(org_id,provider), unique(provider,external_install_id))`.
- New `slack_user_links(id, org_id, user_id→users, slack_user_id, slack_team_id, unique(slack_team_id,slack_user_id))`.
...
- `team_closure_after_reparent()` (AFTER UPDATE OF parent_team_id): cycle guard (RAISE EXCEPTION if new parent is inside the subtree), cut old-ancestor↔subtree edges, stitch new-ancestor × subtree with `super.depth+sub.depth+1`.
...
- RLS: `ENABLE ROW LEVEL SECURITY` + permissive defensive policies on all new tables (documented as inert under service role).
...
**`server/src/services/teamService.ts`** — `createOrg`, `createDepartment`, `createTeam`, `moveTeam` (just `UPDATE parent_team_id`; trigger fixes closure), `deleteTeam` (guard: refuse if children exist unless `reparentChildrenTo` provided), `addMember`/`updateMemberRole`/`removeMember`, `rebuildClosure(orgId)` (ops/repair via recursive CTE).

**`server/src/services/goalService.ts`** — CRUD for goals/KRs/assignees/updates, `setParent` (run cycle-guard CTE before write), and **`recomputeGoalProgress(goalId, {db, visited})`**: if children exist ⇒ weighted avg of children; else by mode (manual=stored; key_results=weighted mean of `clamp01((current-start)/(target-start))`; linked_items=`sum(weight where is_done)/sum(weight)`); write only if changed; then walk **up** to `parent_goal_id` (cycle-safe via `visited`). Fire-and-forget from handlers (mirror `invalidateMonthlySummary(...).catch()` in `entries.ts`).
...
**`server/src/routes/integrations/jira.ts`** — `/api/integrations/jira/connect` (302 to Atlassian 3LO: scopes `read:jira-work read:jira-user offline_access`, signed `state`), `/callback` (exchange code, call `accessible-resources` for `cloudId`, encrypt + store access/refresh in `user_integrations` with `config={cloudId,siteUrl}`), `/disconnect`. Handle **rotating refresh tokens** (store the new refresh token on every refresh; proactive refresh when `<60s` left) via a shared `getJiraClient(userId)` in `server/src/lib/jiraAdapter.ts`.
...
**`server/src/lib/githubApp.ts`** — GitHub App install callback → store `installation.id` in `org_integrations(provider='github_app')`; mint installation tokens for org-level reads.
...
Implement the orchestrator from `slack-git-auto-integration-plan.md`, adding JIRA: `server/src/lib/activityNormalizer.ts` (unified `NormalizedActivity`, dedupe by source URL, week grouping), `autoLogSynthesizer.ts` (Mistral `chat.complete` → 200–400-word draft), and `server/src/jobs/weeklySyncJob.ts` (Monday 09:00 UTC, per-user budget, per-provider circuit breaker). Adapters fetch the ISO week's commits/PRs (GitHub), issues worked (JIRA), and—if the user opts in—Slack messages. Writes `work_log_entries` with `status='auto-generated'`, `pending_review=true`, and `log_source_references`. Client: `AutoLogReview.tsx` + a Dashboard "Ready for review" card (as specified in the prior plan). Gate all of this behind `integration_preferences.sync_enabled` + explicit opt-in (GDPR notes in the prior plan apply).
...
**Slash commands** `server/src/routes/webhooks/slack.ts`: verify signature → map `(team_id, user_id)` → app user via `slack_user_links` (unmapped ⇒ ephemeral "link your account" deep link that matches Slack email → `users.email`) → **ack within 3s** (`200` + ephemeral "working…") → do work async and POST to `response_url`. Commands: `/worklog` (show/create this week's entry), `/goals` (list active goals + progress), `/goals <id> <n>%` (record a `goal_update`, `recomputeGoalProgress`, reply). All mutations re-check `canEditGoal`. Dedupe via `integration_events` on `trigger_id`/`event_id`.
