...
- Reuse `user_integrations` from the prior plan (per-user JIRA + GitHub). Add `jira` to its provider CHECK and ensure `access_token`/`refresh_token` columns store ciphertext (see crypto). Add `is_refreshing BOOLEAN DEFAULT false` and `refresh_started_at TIMESTAMPTZ` columns to support lock-free refresh serialization.
...
- New `org_integrations(id, org_id→organizations ON DELETE CASCADE, provider slack|github_app|jira, external_install_id, bot_token_enc, access_token_enc, refresh_token_enc, webhook_secret, config jsonb, installed_by→users, is_active bool, is_refreshing bool default false, refresh_started_at timestamptz, timestamps, unique(org_id,provider), unique(provider,external_install_id))` + composite FK `(org_id, installed_by) REFERENCES org_members(org_id, user_id) ON DELETE RESTRICT`. Note that for JIRA, `org_integrations` stores the tenant's JIRA Cloud site connection and its shared webhook secret, as well as encrypted access/refresh tokens.
...
- `goal_progress_recomputation_trigger` (AFTER INSERT OR UPDATE OR DELETE ON goal_key_results, goal_links, and goals): automatically triggers progress calculations (`recomputeGoalProgress`) inside the database to prevent concurrent lost updates in Node. To prevent infinite recursion, the trigger on `goals` must be restricted to fire only on structural columns (`AFTER UPDATE OF parent_goal_id, progress_mode, rollup_weight ON goals`), and never on `progress` updates. When a goal is reparented or deleted, the trigger must recompute progress for both the new parent (`NEW.parent_goal_id`) and the old parent (`OLD.parent_goal_id`) to prevent stale calculations.
...
2. **Trigger Infinite Recursion (Postgres Bug):**
   - *Original:* The progress trigger on `goals` was scheduled to fire on any `goals` table update, which would trigger progress updates and repeatedly fire itself recursively.
   - *Correction:* Restricted the trigger on `goals` to only fire when structural columns change: `AFTER UPDATE OF parent_goal_id, progress_mode, rollup_weight ON goals`, preventing recursive loops when updating the `progress` column.
