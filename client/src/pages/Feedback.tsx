...
const categories: Array<{
  ...
  },
  {
    ...
    descKey: 'feedback.improvementDesc',
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
...
const starLabelKeys = ['feedback.star1', 'feedback.star2', 'feedback.star3', 'feedback.star4', 'feedback.star5']

const categoryHelpKeys: Record<CategoryValue, string> = {
  bug: 'feedback.bugHelp',
  feature: 'feedback.featureHelp',
  improvement: 'feedback.improvementHelp',
  general: 'feedback.generalHelp',
}

const categoryPlaceholderKeys: Record<CategoryValue, string> = {
  bug: 'feedback.bugPlaceholder',
  feature: 'feedback.featurePlaceholder',
  improvement: 'feedback.improvementPlaceholder',
  general: 'feedback.generalPlaceholder',
}

export default function Feedback() {
  const { t } = useTranslation()
  usePageMeta({ title: t('feedback.pageTitle'), noIndex: true })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!category) {
      setError(t('feedback.errorCategory'))
      return
    }
    if (rating === 0) {
    ...
    if (message.trim().length < 10) {
      setError(t('feedback.errorMinLength'))
      return
    }
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="text-center">
        <div className="inline-flex items-center px-4 py-2 rounded-full glass mb-6">
          ...
          <span className="text-sm text-gray-300">{t('feedback.heroBadge')}</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
          {t('feedback.heroTitleBefore')}<span className="gradient-text">{t('feedback.heroTitleHighlight')}</span>
        </h1>
      ...
      {submitted && (
        <div className="glass-strong rounded-2xl p-8 text-center relative overflow-hidden animate-fadeIn">
          ...
          <div className="relative">
            ...
            <h3 className="text-2xl font-bold text-white mb-2">{t('feedback.thankYou')}</h3>
            <p className="text-gray-400">{t('feedback.thankYouDesc')}</p>
          </div>
        </div>
      )}
            ...
            {categories.map((cat) => (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <h3 className="text-sm font-semibold text-white mb-1">{t(cat.labelKey)}</h3>
                  <p className="text-xs text-gray-500">{t(cat.descKey)}</p>
                </div>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <span className={`text-sm font-medium transition-all duration-300 ${
              ...
              {hoverRating ? t(starLabelKeys[hoverRating - 1]) : rating ? t(starLabelKeys[rating - 1]) : t('feedback.selectRating')}
            </span>
          </div>
        </div>
          ...
          <label htmlFor="feedback-message" className="block text-sm font-medium text-gray-300 mb-3">
            ...
            {selectedCategory && (
              <span className="text-gray-500 font-normal ml-2">
                — {t(categoryHelpKeys[selectedCategory.value])}
              </span>
            )}
          </label>
          <div className="relative">
            <textarea
              ...
              maxLength={2000}
              placeholder={category ? t(categoryPlaceholderKeys[category]) : t('feedback.messagePlaceholder')}
            ...
            <div className="flex justify-between items-center mt-2 px-1">
              <span className={`text-xs ${message.length > 1800 ? 'text-amber-400' : 'text-gray-600'}`}>
                {message.length < 10 && message.length > 0 ? t('feedback.charsNeeded', { count: 10 - message.length }) : ''}
              </span>
              <span className={`text-xs ${message.length > 1800 ? 'text-amber-400' : 'text-gray-600'}`}>
                {message.length}/2000
              </span>
        ...
        <div className="pt-4">
          ...
          >
            ...
            <span className="text-sm font-medium flex items-center gap-2">
              {t('feedback.historyToggle', { action: showHistory ? t('feedback.hideHistory') : t('feedback.viewHistory'), count: history.length })}
                    ...
                    <div key={item.id} className="glass rounded-xl p-5 card-hover">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-medium text-white">{t(catInfo.labelKey)}</span>
